...
class _NopbaitaphvscreenState extends State<Nopbaitaphvscreen> {
  ...
  // =============== SỬA LẠI PHẦN NÀY ===============
  // Hàm parse chuỗi DateTime an toàn cho cả định dạng SQL lẫn ISO kèm/không kèm 'Z'
  DateTime? parseDateSafe(dynamic dateStr) {
    ...
    try {
      ...
      // Xử lý định dạng SQL: "2026-08-14 21:12:00" -> "2026-08-14T21:12:00"
  ...
  bool get isExpired {
    final hanNop = parseDateSafe(widget.hanNop);
    if (hanNop == null) return false;
    
    // So sánh thời gian thực tế không dính lệch múi giờ UTC/Local
    final now = DateTime.now();
    return now.isAfter(hanNop);
  }
  // =============== KẾT THÚC PHẦN SỬA ===============
  ...
  void initState() {
    ...
    // Debug để kiểm tra (có thể xóa sau)
    if (widget.hanNop != null) {
      debugPrint("Han nop raw: ${widget.hanNop}");
      debugPrint("Han nop type: ${widget.hanNop.runtimeType}");
