...
class Themsuathongbaogvscreen extends StatefulWidget {
  final int? idThongBao;
  final int? idKhoaHoc;
  const Themsuathongbaogvscreen({super.key, this.idThongBao, this.idKhoaHoc});
  @override
  State<Themsuathongbaogvscreen> createState() =>
      _ThemSuaThongBaoGVScreenState();
}

class _ThemSuaThongBaoGVScreenState extends State<Themsuathongbaogvscreen> {
    ...
    try {
      ...
      if (isEdit) {
        res = await http.put(
          ...
          body: jsonEncode({"tieuDe": tieuDe, "noiDung": noiDung}),
        );
      } else {
        res = await http.post(
