...
const checkGroupPermission = async (req, res, next) => {
    try {
        ...
        if (user.vaiTro !== 'hocvien' && user.vaiTro !== 'giangvien') {
            return res.status(403).json({
                success: false,
                error: "Chỉ học viên và giảng viên mới có quyền truy cập nhóm chat"
            })
        }

        req.user = user

        console.log("User group authenticated:", {
            id: user.idNguoiDung,
            hoTen: user.hoTen,
            vaiTro: user.vaiTro
        })
        ...
        console.error("Lỗi check group permission:", error)
        return res.status(403).json({
            success: false,
            error: "Token không hợp lệ hoặc đã hết hạn"
        })
...
export { checkAdmin, checkGiangVien, checkHocVien, checkComment, checkGroupPermission, checkAuth }